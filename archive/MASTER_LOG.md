...
## 第十二轮：鱼缸归属确认 + 行迹像素冒险游戏重写

### 用户指令
1. 鱼缸=主题层全站页尾（非具体页面）——结构核实：唯一 <footer id="footer"> 全局存在，fish.js 注入即在此，归属本就正确，未改
2. 行迹页与旧实现**完全相反的外观与功能** → 用户三选一拍板：像素冒险游戏地图

### 实现（source/_pages-html/map.html 全量重写，415→319 行更精简）
| 维度 | 旧实现 | 新实现（完全反转） |
|------|--------|------------------|
| 外观 | 浅色宣纸水墨卷轴 | 像素大陆：程序化草甸/湖泊/樱花树/松树（确定性种子），梅紫硬边取景框+暖纸内衬 |
| 功能 | 自由拖拽缩放看图+点位读诗 | **主动探险收集制**：WASD/方向键行走、点击 BFS 寻路、迷雾遮罩探索、抵达据点触发任务面板读诗笺；未至之地显示"迷雾之外"悬念卡 |
| 性能 | 大模糊滤镜+每帧全量绘制 | 地形一次性烘焙离屏 + 雾层缓存(局部失效) + 单 canvas 循环 |
| 契约 | places.json | 不变（name/x/y/visited/date/poem/poemTitle/note）；CMS「行迹」数据编辑继续可用 |

### 主题外壳保持
data.layout='page' + top_img=moments.jpg + aside 默认 —— 封面/侧栏/页尾三特征在位

### 验证
/map/ 200 ｜ 三特征 1/1/1 ｜ 脚本 new Function 零错误 ｜ canvas/HUD/任务面板标记齐全 ｜ 已推送 d13891b
### 备注
- 视觉细节（手感和观感）需真机浏览器确认，jsdom 无法覆盖 canvas 渲染
- 修复过程自纠两处：stepTo 双动画残留、tweenTo 死代码

---

## 第十三轮：双鱼缸根治 · 地图六域硬边界 · CMS 可视化建点

1. **鱼缸全站唯一**：自定义页的 `<footer class="*-foot">` 与主题 `<footer id="footer">` 撞名，fish.js 的 `$("footer")` 给每页注入了第二个鱼缸（即用户所见"页面底部黑白运动带"）。七个装饰 footer 全部降级为 div → 全站每页恰 1 个 footer ✓
2. **地图六域分区**：bake() 重写为最近种子+抖动算法 → 西域赭/漠北草/中原青绿/江南花青/岭南苍绿/东海六大色域，异域相邻处画深色锯齿硬边界（非平滑），竖排区域铭牌 DOM 注入
3. **CMS 行迹点位可视化**：新增「行迹点位」视图——左侧迷你地形图点击选坐标（自动换算 places.json 的 x/y 空间），右侧表单建点；支持查看诗句与删除
4. 过程自纠：heredoc 截断致脚本残缺→锚点截断重接；viewPoem 字符串断行→索引切片修复
...
### A1-A6 独立 grep 验收
- `#A8443C`（赭朱）出现次数：**66 → 4**（仅诗句朱批/印章小面积）
- 鱼/鲸/章鱼/whale/kraken：**全 0**（守护完整）
- 政治色 `#C44D3F / #D4A87A`：**全 0**
- 5 区域名（西北/西南/东北/东南/中原）：**全部就位**
- 13 装饰物类型：bell/bridge/camel/deer/ginseng/kite/mountain/rock/seal/sun/tea/willow
- 满堂花醉诗笺主版：13 处 `gm-flora-list` + 标题
- 脚注 2 行结构保留：line 184 红线 / 186 无红线 / 188 草书
- 路由 /map/：**200**
- 内联脚本：**17293 字节** new Function 零错误

### 实施内容
- 满堂花醉主体：3 列 grid（左便签3首古诗札记 / 中诗笺10首 / 右贴纸统计）
- 满堂花醉诗列表：所有诗按 `date` 排序，双栏展示，城市+日期+诗题+诗句
- 大地图撤离大尺寸（width: 100% on its own row, 移至内容海下方）
- 4 子图重设计装饰：mountain/tea/deer/ginseng/willow/bridge/kite/bell/seal 等极简钢笔线描
- 整体配色降饱和 15%，主色用 `赭朱#A8443C` 仅在小面积出现

### 推送
`a991bfe`（估计）已 push origin/backup/site-snapshot-20260826

---

## 第十三轮修订：5 区域省份硬编码（NE=黑/吉/辽）

### 用户反馈
"东北是不偏不倚地将三个省的地图组装起来的"——指 黑龙江/吉林/辽宁 三省（之前的 centroid 自动匹配导致错位）

### A6 独立验收（python json.loads）
- nw  [西北]: 内蒙古/西藏/甘肃/青海/宁夏/新疆（6 省）
- sw  [西南]: 重庆/四川/贵州/云南/西藏（5 省）
- ne  [东北]: 辽宁/吉林/黑龙江（3 省）✅ 用户指定
- se  [东南]: 上海/江苏/浙江/安徽/福建/江西/湖南/广东/广西/海南/台湾/香港/澳门（13 省）
- c  [中原]: 北京/天津/河北/山西/山东/河南/湖北/陕西（8 省）

### 硬要求
- 鱼/鲸/章鱼/whale/kraken：0
- 政治色 #C44D3F / #D4A87A：0
- 5 区域 tab 全部就位
- 路由 /map/：200
- 内联脚本 17293 字符 new Function 零错误

### 推送
已 push 至 backup/site-snapshot-20260826
